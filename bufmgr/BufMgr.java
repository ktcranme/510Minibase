...
class Clock extends Replacer {
  ...
  public int pick_victim() throws BufferPoolExceededException, PagePinnedException {
    ...
    while (state_bit[head].state != Available) {
      ...
      if (num == 2 * numBuffers) {
        // System.out.println("BUFFER STATE: " + getNumUnpinnedBuffers() + "/" + numBuffers);
        // System.out.println("NUM: " + num + " == " + " 2 * " + numBuffers);
...
public class BufMgr implements GlobalConst {
  private HashMap<Integer, Exception> cul;
  ...
  public void culprit() {
    for (int i = 0; i < numBuffers; i++) {
      if (frmeTable[i].pin_count() != 0) {
        System.out.println("Page " + frmeTable[i].pageNo.pid + " still pinned: " + frmeTable[i].pin_count());
        Exception e = cul.get(frmeTable[i].pageNo.pid);
        if (e != null)
        e.printStackTrace();
      }
    }
  ...
  public void flushAll() throws HashOperationException, PageUnpinnedException,
...

  {
    ...
    cul = new HashMap<Integer, Exception>();
    numBuffers = numbufs;
  ...
  public void pinPage(PageId pin_pgid, Page page, boolean emptyPage)
    ...
    try {
      ...
      cul.put(pin_pgid.pid, e);
    }
    // System.out.println("PIN PAGE: " + pin_pgid.pid + "BUFFER STATE: " + getNumUnpinnedBuffers() + "/" + numBuffers);
    // if (pin_pgid.pid > 100) {
    //   try {
    //     throw new Exception();
    //   } catch (Exception e) {
  ...
  public void unpinPage(PageId PageId_in_a_DB, boolean dirty)
    ...
    if (frmeTable[frameNo].pageNo.pid == INVALID_PAGE) {
      ...
      System.out.println("BUFMGR: BAD_FRAMENO.");
      return;
    }
    ...
    if (dirty == true)
      ...
      // System.out.println("UNPEEN: " + PageId_in_a_DB.pid + "BUFFER STATE: " + getNumUnpinnedBuffers() + "/" + numBuffers);
      // if (PageId_in_a_DB.pid > 100) {
      //   try {
      //     throw new Exception();
      //   } catch (Exception e) {
      //     e.printStackTrace();
