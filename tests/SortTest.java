...
class SORTDriver extends TestDriver implements GlobalConst {
  ...
  private static String data1[] = { "raghu", "xbao", "cychan", "leela", "ketola", "soma", "ulloa", "dhanoa", "dsilva",
      ...
      "heitzman", "wan", "gunawan", "djensen", "juei-wen", "josephin", "harimin", "xin", "zmudzin", "feldmann", "joon",
      "wawrzon", "yi-chun", "wenchao", "seo", "karsono", "dwiyono", "ginther", "keeler", "peter", "lukas", "edwards",
      "mirwais", "schleis", "haris", "meyers", "azat", "shun-kit", "robert", "markert", "wlau", "honghu", "guangshu",
      "chingju", "bradw", "andyw", "gray", "vharvey", "awny", "savoy", "meltz" };

  private static String data2[] = { "andyw", "awny", "azat", "barthel", "binh", "bloch", "bradw", "chingju", "chui",
      "chung-pi", "cychan", "dai", "daode", "dhanoa", "dissoswa", "djensen", "dsilva", "dwiyono", "edwards", "evgueni",
      "feldmann", "flechtne", "frankief", "ginther", "gray", "guangshu", "gunawan", "hai", "handi", "harimin", "haris",
      "he", "heitzman", "honghu", "huxtable", "ireland", "jhowe", "joon", "josephin", "joyce", "jsong", "juei-wen",
      "karsono", "keeler", "ketola", "kinc", "kurniawa", "leela", "lukas", "mak", "marc", "markert", "meltz", "meyers",
      "mirwais", "muerle", "muthiah", "neuman", "newell", "peter", "raghu", "randal", "rathgebe", "robert", "savoy",
      "schiesl", "schleis", "scottc", "seo", "shi", "shun-kit", "siddiqui", "soma", "sonthi", "sungk", "susanc", "tak",
      "thiodore", "ulloa", "vharvey", "waic", "wan", "wawrzon", "wenchao", "wlau", "xbao", "xiaoming", "xin", "yi-chun",
      "yiching", "yuc", "yung", "yuvadee", "zmudzin" };
  ...
  protected boolean test1() {
    ...
    attrSize[0] = MAXROWLABELSIZE;
    attrSize[1] = MAXCOLUMNLABELSIZE;
    attrSize[2] = MAXVALUESIZE;
  ...
  protected boolean test2() {
    ...
    // create an iterator by open a file stream

    FileStream fstream = null;

    try {
      fstream = new FileStream("test2.in", null);
    } catch (Exception e) {
      status = FAIL;
      e.printStackTrace();
    }
    ...
    Sort sort = null;
    try {
      sort = new Sort(attrType, (short) 1, attrSize, fstream, 1, order[1], REC_LEN1, SORTPGNUM);
    } catch (Exception e) {
      status = FAIL;
      e.printStackTrace();
    }
  ...
  protected boolean test3() {
    ...
    for (int i = 0; i < LARGE; i++) {
      ...
      try {
        m.setRowLabel(data1[i % NUM_RECORDS]);
        m.setTimeStamp(inum);
        //t.setFloFld(4, fnum);
      } catch (Exception e) {
        status = FAIL;
    ...
    try {
      fStream = new FileStream("test3.in", null);
    } catch (Exception e) {
      status = FAIL;
      e.printStackTrace();
    }
    ...
    Sort sort = null;
    try {
      sort = new Sort(attrType, (short) 4, attrSize, fStream, 3, order[0], 4, SORTPGNUM);
    } catch (Exception e) {
      status = FAIL;
      e.printStackTrace();
    }
