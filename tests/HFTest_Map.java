...
class HFDriver_Map extends TestDriver implements GlobalConst
{
...
public boolean runTests () throws IOException {
  ...
  protected boolean test1 () throws IOException {
    ...
    // inserting map 
    Map rec = new Map();
    rec.setRowLabel("Devesh" + i);
    rec.setColumnLabel("DBMSI");
    rec.setValue("A");
    rec.setTimeStamp(i);
	...
	if (status == OK && !done) {
	  ...
	  String name = ("Devesh" + i );
	  
	  if( (rec.getTimeStamp() != i)
	      || (!name.equals(rec.getRowLabel())) ) {
	    System.err.println ("*** Record " + i
				+ " differs from what we inserted\n");
	    System.err.println ("rec.ival: "+ rec.getTimeStamp()
				+ " should be " + i + "\n");
	    System.err.println ("rec.name: " + rec.getRowLabel()
				+ " should be " + name + "\n");
	    status = FAIL;
	    break;
	  }
...
public class HFTest_Map {

   public static void main (String argv[]) throws IOException {

     HFDriver_Map hd = new HFDriver_Map();
     boolean dbstatus;
